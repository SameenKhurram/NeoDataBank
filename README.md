...
--Creating Table Reions
CREATE TABLE regions (
region_id INTEGER,
region_name VARCHAR(9)
);
Inserting Data in Table Regions
INSERT INTO regions
...
--Inserting data into Nodes Table
...
Select r.region_name, count(c.node_id) as number_of_nodes
from customer_nodes c join regions r on c.region_id = r.region_id
group by region_name;

#How many customers are allocated to each region?
Select r.region_name, count(distinct c.customer_id) as number_of_customers
from customer_nodes c join regions r on c.region_id = r.region_id
group by region_name;

#How many days on average are customers reallocated to a different node?
with daysdifference as -- cte for difference in days
(select customer_id, node_id, DATEDIFF(end_date,start_date) as days_difference
from customer_nodes
where end_date not like '%9999%'
)
Select node_id, round(avg(days_difference),1) as Avg_of_days_difference  --Cte for calculating average of difference in days
from daysdifference
group by node_id 
order by node_id ASC;

#What is the median, 80th and 95th percentile for this same reallocation days metric for each region?
with rows_ as (     --cte for rows with excluding wrong date formats like dates containing year "9999"
...
SELECT region_name,    -- Cte for calculating percentiles
...
select txn_type,count(customer_id) as TotalCount, sum(txn_amount) as TotalAmount
from customer_transactions
group by 1;

#What is the average total historical deposit counts and amounts for all customers?
select customer_id,count(txn_type) as TotalCount, sum(txn_amount) as TotalAmount
from customer_transactions
...
WITH a AS ( --cte for transaction amount
    SELECT 
        customer_id, 
        MONTH(txn_date) AS MonthNumber,
...
SELECT     -- cte for customers with above mentioned condition
    MonthNumber, 
...
select customer_id, month(txn_date) as MonthNumber,                        --cte for closing balance
sum(case when txn_type='deposit' then txn_amount else -txn_amount end) as ClosingBalance
from customer_transactions
group by customer_id, month(txn_date)
order by customer_id;

#What is the percentage of customers who increase their closing balance by more than 5%?
WITH monthly_transactions AS (                 --cte for transaction amount
	SELECT customer_id,
	       Month(txn_date) AS MonthNumber, 
	       SUM(CASE WHEN txn_type IN ('withdrawal', 'purchase') THEN -txn_amount
			ELSE txn_amount END) AS transactions
	FROM customer_transactions
	GROUP BY customer_id, MonthNumber
),
closing_balances AS (           ----cte for closing balance
	SELECT customer_id,
	       MonthNumber,
	       SUM(transactions) OVER(PARTITION BY customer_id ORDER BY MonthNumber) AS closing_balance 
	FROM monthly_transactions
), pct_increase AS (
  SELECT customer_id,           -- cte for difference in balance with previous month
         MonthNumber,
         closing_balance,
         LAG(closing_balance) OVER (PARTITION BY customer_id ORDER BY MonthNumber) AS prev_closing_balance,
         100 * (closing_balance - LAG(closing_balance) OVER (PARTITION BY customer_id ORDER BY MonthNumber)) / NULLIF(LAG(closing_balance) OVER (PARTITION BY customer_id ORDER BY MonthNumber), 0) AS pct_increase
FROM closing_balances
)
SELECT round((100.0 * COUNT(DISTINCT customer_id) / (SELECT COUNT(DISTINCT customer_id) FROM customer_transactions)),2 ) AS pct_customers_with_increase
FROM pct_increase
...
customer_balance AS (
    SELECT 
        ...
        SUM(net_transaction_amount) OVER (
...
Monthend_balance AS (
    SELECT 
        customer_id,
        txn_month,
        MAX(customer_balance) AS month_end_balance
...
running_customer_balance_ AS
(
	SELECT customer_id,
	       txn_month,
	       transaction_amt,
	       SUM(transaction_amt) OVER(PARTITION BY customer_id ORDER BY txn_month) AS running_customer_balance
	FROM transaction_amount)
Select customer_id, txn_month, max(running_customer_balance) as MAX_RCB, min(running_customer_balance) as MIN_RCB, avg(running_customer_balance) as AVG_RCB
from running_customer_balance_
group by customer_id, txn_month;
...
DailyInterest AS (
    SELECT
        ...
        txn_date,
        daily_balance,
        daily_balance * (0.06 / 365) AS daily_interest
    FROM 
        DailyBalances
)
SELECT 
    customer_id,
    Month(txn_date) as MonthNumber,
    round(SUM(daily_interest),2) AS monthly_data_requirement
FROM 
    DailyInterest
GROUP BY 
    customer_id, MonthNumber
